...
onBeforeMount(() => {
  ...
  Axios2.getRechargeCurrencyList().then((res) => {
    ...
    // 自定义排序：JPY 第一位，CNY 最后一位，其他按原顺序
    const sortedList = [];
    const jpyItem = mappedList.find(item => item.name === 'JPY');
    const cnyItem = mappedList.find(item => item.name === 'CNY');
    const otherItems = mappedList.filter(item => item.name !== 'JPY' && item.name !== 'CNY');
    
    // JPY 放第一位
    if (jpyItem) sortedList.push(jpyItem);
    // 其他货币
    sortedList.push(...otherItems);
    // CNY 放最后一位
    if (cnyItem) sortedList.push(cnyItem);
    
    currencyList.value = sortedList;
  });
});
...
const handleSubmit = () => {
  ...
  // 如果国籍为空，使用默认值 "JP"（日本）
  payInfo.value.nationality = nationality.value || "JP";
