...
export default {
  ...
  props: {
    ...
    marketTypeParam: {
      type: String,
      default: "", // RWA页面专用，用于区分市场类型（stock/forex/crypto）
    },
  },
  data() {
    return {
  ...
  watch: {
    ...
    stocksCurrency(val, oldval) {
      if (this.pageType === "stocks") {
        this.$nextTick(() => {
          this.updateCurrency(val, this.pageType);
        });
      }
  ...
  async mounted() {
    // RWA 页面动态获取 K 线类型
    if (this.marketTypeParam) {
      await this.loadRWAKlineTypes();
    } else {
      // 非 RWA 页面使用原有逻辑
  ...
  methods: {
    ...
    // RWA 页面加载 K 线类型
    async loadRWAKlineTypes() {
      try {
        const res = await rwaApi.getRWAKlineTypes();
        if (res.success && res.data && res.data.kline_types) {
          // K线类型映射表（数字 -> 字符串）
          const klineTypeReverseMap = {
            1: "minHour", // 分时
            2: "1min", // 1分钟
            3: "5min", // 5分钟
            4: "15min", // 15分钟
          ...
          // 时间类型中文名称映射
          const timeNameMap = {
            1: this.$t("message.jiaoyi.fenshi"),
            2: this.$t("message.jiaoyi.1fenzhong"),
            3: this.$t("message.jiaoyi.5fenzhong"),
            4: this.$t("message.jiaoyi.15fenzhong"),
            5: this.$t("message.jiaoyi.30fenzhong"),
            6: this.$t("message.jiaoyi.1xiaoshi"),
            7: this.$t("message.jiaoyi.1ri"),
            8: this.$t("message.jiaoyi.1zhou"),
            9: this.$t("message.jiaoyi.1yue"),
          };

          // 转换成页面需要的格式
          this.kTimeArr = res.data.kline_types.map((type) => ({
            data: klineTypeReverseMap[type] || "1day",
            title: timeNameMap[type] || type,
          }));
        ...
        console.error("获取RWA K线类型失败:", error);
        // 失败时使用默认的ETF K线配置
        this.kTimeArr = this.kTimeArrETF;
        this.kTimeIndex = "1day";
        this.kParamsTime = "1day";
    ...
    async getRealTimeData(val, search) {
      ...
      if (res.code == "0") {
        ...
        if (this.sortIndex == 3) {
          this.allListData = this.allListData.sort(this.orderListAsc("close"));
        }
        if (this.sortIndex == 4) {
          this.allListData = this.allListData.sort(
    ...
    getKlineData() {
      ...
      // 判断是否为 RWA 页面（通过 marketTypeParam 判断）
      if (this.marketTypeParam) {
        // RWA 页面使用新 API
        // K线时间类型映射表（字符串 -> 数字）
        const klineTypeMap = {
        ...
        marketConfigApi
          .getKlineData({
            code: symbol,
            klineType: 8 || klineTypeMap[this.kParamsTime],
            marketType: this.marketTypeParam,
            num: 100,
          })
          .then((res) => {
            if (this.mapIndex == 0) {
            ...
            if (res.success && res.data && res.data.kline_list) {
              // 新 API 返回格式转换
              this.KLInedata = res.data.kline_list.map((val) => {
                return {
                  close: parseFloat(val.close_price),
                  high: parseFloat(val.high_price),
                  low: parseFloat(val.low_price),
                  open: parseFloat(val.open_price),
                  timestamp: parseInt(val.timestamp) * 1000, // 秒转毫秒
