...
export default {
  ...
  methods: {
    ...
    remoteSearchRealTime(val) {
      ...
      Axios.getRealtime({ symbol: val }, this.controller)
        .then((res) => {
          if (res.code == "0") {
    ...
    getKlineData() {
      ...
      // 如果上一次请求还在进行中,跳过本次请求
      if (this.isLoadingKline) {
        return;
      }

      this.isLoadingKline = true;
      AxiosKline.getKline({
    ...
    async getRealTimeData(val) {
      // 如果上一次请求还在进行中,跳过本次请求
      if (this.isLoadingRealtime) {
        return;
      }

      this.isLoadingRealtime = true;
      try {
...
.constract-page {
  ...
  * {
    ...
    &::-webkit-scrollbar-thumb {
      background: linear-gradient(180deg, #3a3f4a 0%, #2a2f3a 100%);
      border-radius: 3px;
