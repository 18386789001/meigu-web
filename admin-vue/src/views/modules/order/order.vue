      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button
         type="danger"
          icon="el-icon-delete"
          size="small"
          v-if="isAuth('order:order:make')"
          @click.stop="cancelHandle(scope.row.orderNo)"
          >取消订单
        </el-button>
      </template>
    </avue-crud>
...
export default {
  ...
  methods: {
    ...
    // 取消操作
    cancelHandle(orderNo) {
      this.$confirm("您确认要取消操作吗", "是否确认取消", {
        //系统管理用户是否已绑定
        distinguishCancelAndClose: true,
        confirmButtonText: "取消",
        cancelButtonText: "确定",
        ...
        .catch((action) => {
          if (action === "cancel") {
            this.$http({
              url: this.$http.adornUrl("/exchangeApplyOrder/cancel"),
              method: "post",
              data: this.$http.adornData({
                orderNo: orderNo,
              }),
