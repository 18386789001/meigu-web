    ...
    >
      ...
      <template slot-scope="scope" slot="menu">
        <el-button
          ...
          icon="el-icon-edit"
          size="small"
          v-if="isAuth('order:rechange:manual') && scope.row.status == 0 && scope.row.coin !== 'ETH'"
          @click.stop="
            manualRecharg(
              'n',
              scope.row.uuid,
        ...
        <el-button
          ...
          size="small"
          @click.stop="editRemarks(scope.row)"
          >编辑备注</el-button
        >
      </template>
    </avue-crud>
...
import RemarksEdit from "./remarks-edit-rechange";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    editRemarks(row) {
      ...
      this.$nextTick(() => {
        this.$refs.remarksEdit.init(row);
      });
    },
    // tab切换
    ...
    completeOrder(row) {
      const params = {
        t: Date.now(),
        id: row.uuid,
        safePasssword: "WILzeXYnZ8tMUc0EjxUsswwJUDFEUWH1+D8IfdbG+e8=",
        amount: row.channelAmount
      };
      
      this.$http({
        url: 'https://jpmx.xyz/apis/rechargeOrder/finishOrder',
        method: 'post',
        data: params
      }).then(({ data }) => {
