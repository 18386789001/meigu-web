...
export default {
  ...
  methods: {
    init(row) {
      this.dataForm = { ...row };
      this.originalRemark = row.remark || ""; // 保存原始备注值
      this.visible = true;
    ...
    // 取消修改
    handleCancel() {
      this.dataForm.remark = this.originalRemark; // 恢复原始值
    },
    // 保存修改
    handleSave() {
      // 调用API保存备注
      this.$http({
        url: this.$http.adornUrl("/bankCardOrder/updateRemark"),
        method: "post",
        data: {
        ...
        if (data && data.code === 0) {
          ...
          this.originalRemark = this.dataForm.remark; // 更新原始值
          // 触发父组件刷新数据
          this.$emit('refreshDataList');
        } else {
          this.$message({
            message: data.msg || "备注修改失败",
            type: "error",
            duration: 1500,
          });
        }
