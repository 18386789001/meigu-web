  ...
  <div class="mod-role">
    <avue-crud ref="crud" :page.sync="page" :data="dataList" :option="tableOption" @search-change="searchChange"
      ...
      <template slot="ndhmSearch">
        <el-input v-model="uid" placeholder="用户名、UID(完整)" clearable></el-input>
      </template>
      <template slot="roleSearch">
        <el-select v-model="roleName" placeholder="请选择" clearable @change="changeVal()">
          <el-option v-for="item in roleList" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </template>
      <template slot="lognthSearch">
        <el-input v-model="log" placeholder="日志" clearable></el-input>
      </template>
      ...
      <template slot="startTimendhSearch">
        <el-date-picker v-model="startTime" type="datetime" valueFormat="yyyy-MM-dd HH:mm:ss" placeholder="选择开始时间">
        </el-date-picker>
      </template>
      <template slot="endTimendhSearch">
        <el-date-picker v-model="endTime" type="datetime" valueFormat="yyyy-MM-dd HH:mm:ss" placeholder="选择结束时间">
        </el-date-picker>
      </template>
      <template slot-scope="scope" slot="menu">
        <el-button type="primary" icon="el-icon-edit" size="small"
          v-if="isAuth('sys-config:account-change-record-sys-config:operate')" @click.stop="moneylog(scope.row)"
          style="margin-bottom: 5px;">{{ scope.row.show * 1 == 1 ? '隐藏前端显示操作' : '展示前端显示操作' }}</el-button>
        <br>
        <el-button type="warning" icon="el-icon-edit-outline" size="small"
          ...
          @click.stop="updateMoneylog(scope.row)">修改日志</el-button>
      </template>
    </avue-crud>
    <!-- 弹窗, 新增 / 修改 -->
    ...
    <!-- 修改日志弹窗 -->
    <el-dialog title="修改日志" :close-on-click-modal="false" :visible.sync="updateLogVisible" width="600px">
      <el-form :model="updateLogForm" ref="updateLogForm" label-width="100px">
        <el-form-item label="当前日志" prop="log">
          <el-input type="textarea" :rows="3" v-model="updateLogForm.log" placeholder="请输入新的日志内容"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="updateLogVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmUpdateLog">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 修改日志
    updateMoneylog(row) {
      this.updateLogForm.uuid = row.uuid;
      this.updateLogForm.log = row.log || '';
      this.updateLogVisible = true;
    },
    // 确认修改日志
    confirmUpdateLog() {
      if (!this.updateLogForm.log) {
        this.$message.error('请输入日志内容');
        return;
      }
      this.$confirm('确定修改日志吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
        .then(() => {
          this.$http({
            url: this.$http.adornUrl('/moneylog/updatMoneyLog'),
            method: 'post',
            data: this.$http.adornData({
              uuid: this.updateLogForm.uuid,
              log: this.updateLogForm.log,
            }, false),
          }).then(({ data }) => {
            if (data.code == 0) {
              this.$message({
