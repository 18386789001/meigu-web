...
import { shouldRedirectToPC } from '@/utils/deviceDetection.js';
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { path: 'futures/detail/:symbol', name: 'FuturesDetail', meta: { tarbar: false }, component: () => import(/* webpackChunkName: "futuresDetail" */ '@/views/futures/Detail.vue') },
          { path: 'commodities', meta: { tarbar: false }, component: () => import(/* webpackChunkName: "commodities" */ '@/views/commodities/List.vue') },
      ...
      },
      {
        path: '/futures',
        name: 'Futures',
        redirect: '/futures/trade',
        meta: {
          tarbar: false,
        },
        component: () => import('@/views/Layout.vue'),
        children: [
          { path: 'trade', name: 'FuturesTrade', component: () => import(/* webpackChunkName: "futuresTrade" */ '@/views/futures/Trade.vue') },
          { path: 'list', name: 'FuturesList', component: () => import(/* webpackChunkName: "futuresList" */ '@/views/futures/List.vue') },
          { path: 'detail/:symbol', name: 'FuturesDetail', component: () => import(/* webpackChunkName: "futuresDetail" */ '@/views/futures/DetailSimple.vue') },
          { path: 'buy', name: 'FuturesBuy', component: () => import(/* webpackChunkName: "futuresBuy" */ '@/views/futures/Buy.vue') },
          { path: 'sell', name: 'FuturesSell', component: () => import(/* webpackChunkName: "futuresSell" */ '@/views/futures/Sell.vue') },
        ]
      },
      {
        path: '/position',
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            component: () =>
              import(
                /* webpackChunkName: "rechargeWithdrawRecord" */ "@/views/Record/DepositAndWithdrawal.vue"
              ),
          },
          {
            //提现页面
...
router.beforeEach((to, from, next) => {
  ...
  if (shouldRedirectToPC()) {
    console.log('检测到PC端设备访问移动端应用，直接跳转到PC端');
    
    // 检查URL参数中是否有跳过跳转的标识
    const urlParams = new URLSearchParams(window.location.search);
    const skipRedirect = urlParams.get('skip_pc_redirect') === 'true';
  ...
  if (window.location.pathname.includes('/syn')) {
    console.log('检测到/syn路径访问，这是wap-vue应用');
    // 确保路由正确重定向到首页，但允许登录和注册页面正常访问
