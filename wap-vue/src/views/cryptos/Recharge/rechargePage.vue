        ...
        <div class="rechargePage box-border px-8 text-28">
            ...
            <div class="pl-8 pt-8 pr-8 text-center flex flex-col items-center justify-center mt40">
                <div>
                    ...
                    <img :src="currentQRCodeImg" alt="QR Code" class="QRcodeImg" />
                </div>
                <div class="code-btn btnMain text-center text-26 text-white" @click="downloadCurrentQRCode">{{
                    $t('保存二维码') }}</div>
            </div>
...
import trc20QRCodeImage from '@/assets/TRC20二维码.png';
...
export default {
    ...
    data() {
        return {
            ...
            // 硬编码的TRC20二维码图片
            trc20QRCodeImg: trc20QRCodeImage,
            // 接口返回的二维码图片
            apiQRCodeImg: ""
        }
    },
    computed: {
        ...
        currentQRCodeImg() {
            // 优先使用接口返回的二维码图片
            if (this.apiQRCodeImg) {
                return this.apiQRCodeImg;
            }
            // 否则根据链名称使用静态图片
            if (this.blockchain_name === 'TRC20') {
                return this.trc20QRCodeImg;
            } else {
                return this.ethQRCodeImg;
            }
        }
    },
    mounted() {
    ...
    methods: {
        ...
        getData() {
            Axios.getBlock({
                ...
                // 优先使用接口返回的二维码图片
                this.apiQRCodeImg = res[0].imgsrc || "";
                this.getQRCode();
        ...
        changeBlockchain(index) {
            ...
            // 切换链时更新二维码图片
            this.apiQRCodeImg = this.chainList[this.blockchainIndex].imgsrc || "";
            this.getQRCode();
        },
        getToken() {
        ...
        downloadCurrentQRCode() {
            ...
            const el = document.createElement('a');
            // 根据链名称设置对应的二维码图片
            el.href = this.currentQRCodeImg;
            // 根据是否有接口返回的二维码来设置文件名
            let fileName;
            if (this.apiQRCodeImg) {
                // 使用接口返回的二维码时，用币种和链名称组合
                fileName = `${this.coin}-${this.blockchain_name}-qrcode.png`;
            } else {
                // 使用静态二维码时，根据链名称设置
                fileName = this.blockchain_name === 'TRC20' ? 'trc20-qrcode.png' : 'eth-qrcode.png';
            }
