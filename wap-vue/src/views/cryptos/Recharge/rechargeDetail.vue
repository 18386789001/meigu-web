    ...
    <div class="rechargeDetail">
        ...
        <div class="text-30 pl-30 pr-30 mt53">
            ...
            <div class="flex justify-between mb74">
                ...
                <div class="textColor">{{ getTranslatedFailureMsg(info.failure_msg) }}</div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        // 翻译failure_msg中的中文硬编码
        getTranslatedFailureMsg(failureMsg) {
            if (!failureMsg) {
                return '-'
            }
            
            // 检查是否包含特定的中文消息
            if (failureMsg.includes('当前币种为eth转化为USDT,注意查询原有付款地址。')) {
                return this.$t('ethConvertedToUsdtNotice')
            }
            
            // 如果有其他中文消息，可以在这里添加更多翻译规则
            // 暂时返回原始消息
            return failureMsg
        }
    }
