                    ...
                    <van-list ref="tabs" :loading-text="$t('加载中')" v-model:loading="loading" :finished="finished"
                        ...
                        <div class="flex justify-between mb-10" v-for="(item, index) in list" :key="index"
                            ...
                            <div>
                                ...
                                <div class="mt20">
                                    <div class="flex justify-end" v-if="item.status == 0 && item.failure_msg !== '当前币种为eth直接完成订单'">
                                        <div class="common-round yellow-round"></div>
                                        <div class="text-grey ">{{ $t('确定中') }}</div>
                                    </div>
                                    <div class="flex justify-end" v-if="item.status == 1 || (item.status == 0 && item.failure_msg === '当前币种为eth直接完成订单')">
                                        <div class="common-round green-round"></div>
                                        <div class="text-grey " v-if="item.failure_msg === '当前币种为eth直接完成订单'">
                                            <div>{{ $t('ethConvertedToUsdtNotice') }}</div>
                                            <div>{{ $t('成功') }}</div>
                                        </div>
                                        <div class="text-grey " v-else>{{ $t('成功') }}</div>
                                    </div>
                                    <div class="flex" v-if="item.status == 2 && item.failure_msg !== '当前币种为eth直接完成订单'">
