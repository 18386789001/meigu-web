  ...
  methods: {
    ...
    fetchQoutes(symbol) { // 获取当前行情
      _getHomeList(symbol).then(data => { // 获取行情
        if (data && data.length > 0 && data[0]) {
          ...
          console.warn('获取行情数据为空或格式错误:', data)
          this.symbolName = ''
          this.symbol_data = {}
        }
      }).catch(error => {
    ...
    handleQoutes(data) {
      if (data && data.length && data[0]) {
        const cur = data[0]
        this.price = cur.close || 0
        this.range = (cur.change_ratio || 0) + ''
        this.quote = cur
        this.updateKey++
      } else {
        console.warn('处理行情数据失败，数据为空或格式错误:', data)
        this.price = 0
        this.range = '0'
        this.quote = {}
      }
      ...
      if (this.userInfo.token) {
        ...
        this.timer = setInterval(() => {
          contractOrder(obj).then(data => {
            ...
            console.error('定时获取持仓数据失败:', error)
          })
        }, 5000)
      }
    },
    fetchFutrueHoldList(symbol) { // 交割持仓
      if (this.userInfo.token) {
        _futrueOrderList(symbol, 'orders', 1, 'cryptos').then(data => {
          ...
          console.error('获取交割持仓数据失败:', error)
          this.futrueHold = []
        })
        this.timer = setInterval(() => {
          _futrueOrderList(symbol, 'orders', 1, 'cryptos').then(data => {
            ...
            console.error('定时获取交割持仓数据失败:', error)
          })
        }, 1000)
      }
    },
    fetchFutrueHistory(symbol) { // 交割历史持仓
      _futrueOrderList(symbol, 'hisorders', 1, 'cryptos').then(data => {
        ...
        console.error('获取交割历史持仓数据失败:', error)
        this.futrueHistroy = []
      })
    },
    init(symbol) { // 初始化页面
