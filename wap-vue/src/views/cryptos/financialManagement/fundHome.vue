      ...
      <div class="px-8 mt-10 pb-20">
        <div v-for="(item, index) in list" :key="index"
          ...
          <div class="flex justify-center items-center">
            <img class="w-32 h-32 mr-5" src="https://file.jpmx.xyz/trading-order-roseccc/file/2025-10-03/d4c8048b-9259-4467-88fa-dd137ca92972blob.png" alt="">
...
export default {
  ...
  activated() {
    getfinacialProductsList().then(res => {
      console.log('理财列表API返回数据:', res)
      // 检查并处理img字段
      this.list = res.map(item => ({
        ...item,
        img: item.img && !item.img.includes('logo.png') ? item.img : '/logo.jpg'
      }))
