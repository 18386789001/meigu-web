    ...
    <fx-header :title="$t('my')" :showLeft="false">
      <template v-slot:right>
        ...
        <van-badge class="w-35 h-33 mr-22" :content="unreadMsg">
          <van-icon name="service-o" size="18" @click="openCustomerService" />
        </van-badge>
      </template>
    </fx-header>
    ...
    <div class="mt-4">
      <van-cell-group v-for="(item, index) in cellList" :key="index" :title="item.title" :border="false">
        <van-cell v-for="(_item, _index) in item.list" :key="_index" is-link="is-link" center="center"
          :title="_item.title" @click="_item.isThemeSwitch ? toggleTheme() : (_item.path === '/customerService' ? openCustomerService() : onRoute(_item.path))">
          <template #icon>
    ...
    : kycHighStatus == 2
      ...
      : kycHighStatus == 3
        ...
        : ""
          ...
          <div v-if="_item.isThemeSwitch">
            <span class="theme-status">{{ thStore.theme === 'dark' ? $t('darkMode') : $t('lightMode') }}</span>
          </div>
        </van-cell>
      </van-cell-group>
    </div>
...
// 主题切换函数
const toggleTheme = () => {
  const newTheme = thStore.theme === 'dark' ? 'white' : 'dark';
