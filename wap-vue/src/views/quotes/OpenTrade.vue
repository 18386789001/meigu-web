      ...
      <header class="header">
        <div class="flex-l">
          ...
          <p class="title">
            ...
            <span class="text">{{ getDisplayTitle() }}</span>
          </p>
        </div>
    ...
    fetch('https://jpmx.xyz/api/item!list.action?type=commodities&language=zh-CN')
      ...
      .then(apiData => {
        if (apiData && apiData.data && Array.isArray(apiData.data)) {
          // 处理API返回的商品数据，隐藏GOLD和Silver，显示XAUUSD和XAGUSD
          const processedList = apiData.data
            .filter(apiItem => apiItem.symbol !== 'Nickel' && apiItem.symbol !== 'Zinc' && apiItem.symbol !== 'GOLD' && apiItem.symbol !== 'Silver')
            .map(apiItem => {
              // 从name字段中提取中文名称和交易符号
              const nameMatch = apiItem.name.match(/^([^（(]+)（([^）]+)）/);
              if (nameMatch) {
                const chineseName = nameMatch[1].trim();
                const tradingSymbol = nameMatch[2].trim();
                
                // 使用i18n翻译中文名称
                const translatedName = t(chineseName);
                
                return {
                  ...apiItem,
                  name: `${translatedName}（${tradingSymbol}）`,
...
const fetchList = () => { // 获取行情
  if (symbolType === 'commodities') {
    // 对于commodities类型，使用_getCoins获取商品列表（包含中文名称）
    _getCoins({ type: symbolType }).then(list => {
      // 获取价格信息
      ...
      _getHomeList(symbols).then(priceList => {
        ...
        const mergedList = list.map(item => {
          ...
          return {
            ...item,
            name: getCommodityDisplayName(item), // 使用i18n翻译商品名称
...
const getDisplayTitle = () => {
  if (!symbolName.value) return '';
  
  // 如果是commodities类型，显示为"翻译名称 (实际交易符号)"格式
  if (symbolType === 'commodities') {
    // 尝试从API数据中提取实际交易符号
    const actualSymbol = extractSymbolFromName(symbolName.value) || symbol.value;
    return `${symbolName.value} (${actualSymbol})`;
  }
  
  // 非commodities类型，保持原有显示格式
  return `${symbolName.value} (${symbol.value})`;
};
...
const getModalDisplayName = (item) => {
  ...
  if (symbolType === 'commodities') {
    // 尝试从API数据中提取实际交易符号
    const actualSymbol = extractSymbolFromName(item.name) || item.symbol;
    return `${item.name} (${actualSymbol})`;
  }
...
const querySymbolName = () => {
  // 如果是commodities类型，使用hobi!getRealtime.action接口获取中文名称
  if (symbolType === 'commodities') {
    _getHomeList(symbol.value, true).then(data => {
      if (data && data.length > 0) {
        // 使用i18n翻译商品名称
        symbolName.value = getCommodityDisplayName(data[0]) || data[0].name || data[0].enName || symbol.value
      }
    }).catch(() => {
      // 如果API失败，使用symbol作为fallback
      symbolName.value = symbol.value
    })
  } else {
    // 其他类型使用原有逻辑
...
const onRoute = () => {
  // 根据当前的symbol和symbolType跳转到对应的K线图页面
  router.push(`/quotes/detail?symbol=${symbol.value}&type=${symbolType}`)
}
