        ...
        <van-tabs v-model:active="tabActive" shrink @click-tab="onClickTab">
          <van-tab v-for="(item, index) in listTab" :key="item.tabIndex" :name="item.tabIndex" :title="item.title">
            <component @changeLetMego="handleChangeLetMego" :index="item.tabIndex" :tabActive="tabActive"
              :activeTradingTab="route.query.activeTradingTab"
...
components.value.set(
  'MYStock',
  defineAsyncComponent(() => import('../myStock/List.vue'))
)
components.value.set(
  'INDIAStock',
...
components.value.set(
  'Futures',
  defineAsyncComponent(() => import('../futures/List.vue'))
)
components.value.set(
  'Commodities',
  defineAsyncComponent(() => import('../commodities/List.vue'))
)
components.value.set(
  'RWA',
  defineAsyncComponent(() => import('../rwa/List.vue'))
)
const listTab = ref([
  {
    title: t('大宗商品'),
    type: 'Commodities',
    urlMatch: 'commodities',
...
// 存储每个tab的函数
const tabFunctions = ref({})

const handleChangeLetMego = (fn) => {
  // 获取当前tab的索引
  const currentTab = listTab.value.find(item => item.tabIndex === tabActive.value)
  if (currentTab) {
    tabFunctions.value[currentTab.tabIndex] = fn
  }
}

let timer = setInterval(() => {
  // 只调用当前激活tab的函数
  const currentFunction = tabFunctions.value[tabActive.value]
