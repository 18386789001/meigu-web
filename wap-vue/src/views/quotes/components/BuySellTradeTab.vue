  ...
  <section class="inner-tab-container">
    ...
    <div class="orderbook-container flex justify-between px-30 py-30">
      <div
        ...
        <p class="desc" v-if="symbolType !== 'commodities'">
          <span class="label">
              ...
              <ul>
                <li v-for="item in pairsList" :key="item.symbol" class="flex justify-between py-2">
                  <div class="flex items-center">
                    <img :src="getIconSrc(item.symbol)" 
                      class="w-15 h-15 rounded-full mr-5 usdt-icon" 
                      @error="handleImageError($event, item.symbol)"
                      @load="handleImageLoad($event, item.symbol)" />
...
const commodityPriceTimer = ref(null);
...
const initPage = (nv) => {
  ...
  // 对于大宗商品，启动定时器定期从K线API获取最新价格
  if (symbolType.value === 'commodities') {
    startCommodityPriceTimer();
  }
};
...
// 启动大宗商品价格定时器
const startCommodityPriceTimer = () => {
  // 清除现有定时器
  if (commodityPriceTimer.value) {
    clearInterval(commodityPriceTimer.value);
  }
...
// 停止大宗商品价格定时器
const stopCommodityPriceTimer = () => {
  if (commodityPriceTimer.value) {
    clearInterval(commodityPriceTimer.value);
    commodityPriceTimer.value = null;
    console.log('✅ 交易页面停止大宗商品价格定时器');
  }
};
...
const getLatestPriceFromKline = async (symbol) => {
  try {
    console.log('🔄 交易页面开始从K线API获取最新价格，symbol:', symbol);
    ...
    if (data && data.data && Array.isArray(data.data) && data.data.length > 0) {
      ...
      if (timeDiff <= 604800) {
        ...
        if (curNameIndex.value === 0) {
          form.price = latestKlineData.close;
        }
        
        console.log('✅ 交易页面K线API价格更新成功:', {
          symbol: symbol,
        ...
        console.warn('⚠️ 交易页面K线数据时间过旧，时间差:', timeDiff, '秒，不更新价格');
      }
    } else {
      console.warn('⚠️ 交易页面K线API返回数据格式错误或无数据');
    }
  } catch (error) {
    console.error('❌ 交易页面获取K线数据失败:', error);
  }
...
const handleQoutes = (data) => {
  if (data && data.length) {
    ...
    if (symbolType.value === 'commodities') {
      console.log('🏭 交易页面检测到大宗商品，从K线API获取最新价格');
      getLatestPriceFromKline(symbol.value);
    } else {
      // 非大宗商品，使用WebSocket数据
      chartData.value = cur;
...
const getWallet = () => {
  // 对于commodities类型，使用forex作为symbolType
  const apiSymbolType = symbolType === 'commodities' ? 'forex' : symbolType;
...
const getIconSrc = (symbol) => {
  ...
  const iconPath = `/symbol/${symbol.toLowerCase()}.png`;
  console.log('getIconSrc - symbol:', symbol, 'iconPath:', iconPath);
  return iconPath;
};

// 图片加载错误处理
const handleImageError = (event, symbol) => {
  console.warn('图标加载失败:', event.target.src, 'symbol:', symbol);
  // 记录失败的图标
  failedIcons.value.add(symbol.toLowerCase());
  // 替换为默认图标
  event.target.src = '/symbol/default.png';
};

// 图片加载成功处理
const handleImageLoad = (event, symbol) => {
  console.log('图标加载成功:', symbol);
  // 从失败列表中移除（如果存在）
  failedIcons.value.delete(symbol.toLowerCase());
};
onUnmounted(() => {
  closeSocket();
  clearTimer();
  stopCommodityPriceTimer();
});
