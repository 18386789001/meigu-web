        ...
        <div class="modal-inner-box">
          ...
          <div class="sidebar">
            <van-sidebar v-model="activeSideBarIndex">
              ...
              <van-sidebar-item @click="openType('commodities', t('大宗商品'))" :title="`${t('大宗商品')}(${fixedData.commodities || 0})`" />
...
import { _itemUserOptionalList, _itemUserOptionalListAdd, _listItemsById, _itemUserOptionaAddList } from '@/service/quotes.api'
...
const failedIcons = ref(new Set()) // 记录加载失败的图标
...
const getIconSrc = (symbol) => {
  if (!symbol) {
    return `/symbol/default.png`
  }
  
  // 如果图标已经加载失败，直接返回默认图标
  if (failedIcons.value.has(symbol.toLowerCase())) {
    return `/symbol/default.png`
  }
  
  // 优先使用SVG格式，如果不存在再使用PNG格式
...
const getMyCoinsList = () => {
  ...
  const defaultSymbols = [
    { symbol: 'BTC', type: 'cryptos' },
    { symbol: 'ETH', type: 'cryptos' },
    { symbol: 'MSFT', type: 'US-stocks' },
    { symbol: 'AAPL', type: 'US-stocks' }
  ]
  ...
  if (!useStore.userInfo.token) { //没登陆
    ...
    _itemUserOptionalList(params).then(data => {
      ...
      if (optionalList.value.length === 0) {
        createDefaultOptionalList()
      } else {
        activeInfo.value = optionalList.value[0]
        byIdItem(activeInfo.value.listId)
      }
      
      // 无论用户是否有自选列表，都确保显示默认四个品种
      // 将默认品种添加到用户自选列表前面
      const combinedSymbols = [...defaultSymbols]
      if (quotesStore.$state.opcionalList && quotesStore.$state.opcionalList.length > 0) {
        // 过滤掉重复的默认品种，只保留用户添加的品种
        const userSymbols = quotesStore.$state.opcionalList.filter(userSymbol => 
          !defaultSymbols.some(defaultSymbol => defaultSymbol.symbol === userSymbol.symbol)
        )
        combinedSymbols.push(...userSymbols)
      }
      quotesStore[OPCIONA_LIST](combinedSymbols)
    })
  }
}

// 创建默认自选列表（不添加默认品种，因为默认品种总是显示）
const createDefaultOptionalList = async () => {
  try {
    // 创建默认自选列表
    const listParams = {
      listName: 'My Choice',
    ...
    const listResult = await _itemUserOptionaAddList(listParams)
    
    if (listResult && listResult.listId) {
      // 重新获取自选列表
      const data = await _itemUserOptionalList({})
      fixedData.value = data.count
      optionalList.value = data.list || []
    ...
    console.error('创建默认自选列表失败:', error)
  }
}
//根据id获取列表
const byIdItem = (id) => {
  ...
  _listItemsById(obj).then((res) => {
    ...
    // 合并默认品种和用户自选品种，去重
    const combinedSymbols = [...defaultSymbols]
    const userSymbols = arry.filter(userSymbol => 
      !defaultSymbols.some(defaultSymbol => defaultSymbol.symbol === userSymbol.symbol)
    )
    combinedSymbols.push(...userSymbols)
    
    console.log(combinedSymbols, 222)
    quotesStore[OPCIONA_LIST](combinedSymbols)
...
const openByTypeItem = (val, name) => {
  ...
  if (val === 'all') {
    ...
    _listItemsByType(obj).then((res) => {
      ...
      // 合并默认品种和按类型筛选的品种
      const combinedSymbols = [...defaultSymbols]
      const filteredSymbols = arry.filter(symbol => 
        !defaultSymbols.some(defaultSymbol => defaultSymbol.symbol === symbol.symbol)
      )
      combinedSymbols.push(...filteredSymbols)
      
      console.log(combinedSymbols, 2222)
      quotesStore[OPCIONA_LIST](combinedSymbols)
      if (combinedSymbols.length == 0) {
        quotesStore[CITAS_LIST]([])
      }
...
const itemClick = (item) => {
  ...
  } else if (item.type == "commodities") { //大宗商品
    router.push('/quotes/detail?symbol=' + item.symbol + '&isOptional=1&symbolType=commodities&type=commodities')
  } else {
