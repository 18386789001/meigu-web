...
const generateExtranetLink = () => {
  let extranetLink = ''
  // 检查是否使用新的客服系统
  const isNewCustomerService = homesStore.kefu_url && homesStore.kefu_url.includes('cdn.bot04sg.cfd');
  
  if (userStore.userInfo && userStore.userInfo.usercode) {
    if (isNewCustomerService) {
      // 新客服系统的参数格式
      let params = `&userId=${userStore.userInfo.usercode}`;
      if (userStore.userInfo.email) {
        params += `&email=${encodeURIComponent(userStore.userInfo.email)}`;
      }
      if (userStore.userInfo.username) {
        params += `&userName=${encodeURIComponent(userStore.userInfo.username)}`;
      }
