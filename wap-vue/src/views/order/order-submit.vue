    ...
    <div v-if="payInfo.uuid">
      <div class="order-top" v-if="payInfo.state == 0">
        ...
        <div class="flex but-wrap">
          <van-button class="w-full cancel-but" @click="cancelBankCardOrder()">{{ $t('cancelOrder') }} </van-button>
...
// 跳转到新的客服链接
const redirectToCustomerService = async () => {
...
//银行卡取消订单
const cancelBankCardOrder = async () => {
  try {
    // 构建请求参数 - 使用正确的参数格式
    const params = {
      order_no: payInfo.value.orderNo || orderNo.value
    }
    
    console.log('取消订单参数:', params)
    
    // 调用正确的取消订单API
    const result = await c2cOrderApply(params)
