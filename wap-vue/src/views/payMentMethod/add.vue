...
onMounted(async () => {
    ...
    console.log('📋 从 sessionStorage 获取的数据:', data)
...
const getPaymentMethodConfigDetail = () => {
    console.log('🔄 尝试获取支付模板配置，ID:', id.value)

    _paymentMethodConfigDetail({
        id: id.value,
        language: locale.value,
    }).then(data => {
        console.log('✅ 获取支付模板配置成功:', data)
...
const submit = () => {
    if (type.value === 'add') {
        ...
        if (!paramsForm) {
            console.error('❌ 参数格式化失败')
            return
        }

        console.log('📤 提交添加支付方式参数:', paramsForm)
...
const formatParams = (id) => {
    console.log('🔧 格式化提交参数，configDetail:', configDetail.value)
    ...
    if (id == 'method_config_id') {
        ...
        newParams = {
            ...
            method_config_id: configDetail.value.methodConfigId || configDetail.value.uuid,
            real_name: params.value.name,
            remark: params.value.remark,
            qrcode: submitImg.value || ''
        }
