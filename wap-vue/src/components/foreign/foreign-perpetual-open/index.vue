      ...
      <div class="flex">
        <div class="w-full flex flex-col">
          ...
          <div>
            ...
            <!-- 杠杆选择 -->
            <div class="flex justify-between h-16 items-center mb-8">
              <span class="textColor3 text-28">{{ $t('杠杆') }}</span>
              <div class="leverage-selector" @click="showLeverageModal = true">
                <span class="textColor text-28">{{ form.lever_rate }}x</span>
...
let paraIndex = ref(0)
let showLeverageModal = ref(false)
let maxOpenAmount = ref(0)
let form = ref({
  symbol: '', // 币种
  session_token: '',
  direction: 'buy', // 买or卖
...
const handleLeverageUpdate = (leverage) => {
  // 实时更新杠杆值
  console.log('Leverage updated:', leverage)
}
...
const handleLeverageConfirm = (leverage) => {
  ...
  showToast(t('杠杆调整成功'))
  console.log('Leverage confirmed:', leverage)
}

const handleLeverageCancel = () => {
  showLeverageModal.value = false
  console.log('Leverage adjustment cancelled')
}
