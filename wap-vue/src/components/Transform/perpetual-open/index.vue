    ...
    <div class="perpetual-open">
      <div class="flex pt-8">
        ...
        <div class="w-36 h-16 ml-8 rounded tabBackground textColor" v-if="selectIndex / 1 === 1">
          <div @click="showLeverageModal = true" style="height:100%; box-sizing: border-box"
               class="relative word-30 pl-7 pr-2 w-36 h-16 tabBackground select-wrap flex justify-between items-center cursor-pointer">
...
import LeverageModal from '@/components/Transform/leverage-modal/index.vue';
import { fixDate, throttle, getStorage, setStorage } from "@/utils/index.js";
...
export default {
  ...
  computed: {
    ...
    initData() {
      ...
      if (this.selectIndex / 1 === 1) {
        ...
        if (!obj.lever || !obj.lever.length) { // 倍数
          ...
          this.form.lever_rate = cachedLeverRate || obj.lever[0].lever_rate
        }
        return obj
      }
    ...
    coudBuyVolume() { // 可买数量
      // 安全检查：确保initOpen.volume和lever_rate都存在且为有效数字
      if (!this.initOpen || !this.initOpen.volume || isNaN(this.initOpen.volume) ||
          !this.form.lever_rate || isNaN(this.form.lever_rate) || this.form.lever_rate <= 0) {
        return 0;
      }
  ...
  methods: {
    ...
    // 获取张数,数据转换
    getVolumnByLever() {
      // 安全检查：确保initOpen.volume存在且为有效数字
      if (!this.initOpen || !this.initOpen.volume || isNaN(this.initOpen.volume)) {
        return 0;
      }
      let vol = this.initOpen.volume / 1;
    ...
    handleInitSliderOption(amount) {
      ...
      // 安全检查：确保initOpen.volume存在且为有效数字
      if (!this.initOpen || !this.initOpen.volume || isNaN(this.initOpen.volume)) {
        console.log("initOpen.volume无效，设置默认值");
        this.options.max = 0;
